#include "YNARootListController.h"

@implementation YNARootListController

- (void)viewDidLoad {

    [super viewDidLoad];

    self.appearanceSettings = [YNAAppearanceSettings new];
    self.hb_appearanceSettings = [self appearanceSettings];


    self.preferences = [[HBPreferences alloc] initWithIdentifier:@"love.litten.yunapreferences"];
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:@"/Library/MobileSubstrate/DynamicLibraries/Yuna.disabled"]) {
        ...
        UIAlertController* alertController = [UIAlertController alertControllerWithTitle:@"Yuna" message:@"Yuna has detected that you have disabled it with iCleaner Pro, here are some quick actions you can perform" preferredStyle:UIAlertControllerStyleAlert];
...
- (void)resetPrompt {
    ...
    UIAlertController* resetAlert = [UIAlertController alertControllerWithTitle:@"Yuna" message:@"Do you really want to reset your preferences?" preferredStyle:UIAlertControllerStyleActionSheet];
...
- (void)respring {
    ...
    [UIView animateWithDuration:1.0 delay:0.0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        if (![[NSFileManager defaultManager] fileExistsAtPath:@"/Library/MobileSubstrate/DynamicLibraries/shuffle.dylib"])
            ...
            [HBRespringController respringAndReturnTo:[NSURL URLWithString:@"prefs:root=Tweaks&path=Yuna"]];
    }];
